...
export function compute_bot_readiness(bot, { whatsapp_channels = [], instagram_channels = [], config = default_config } = {}) {
  ...
  // 3) IA real: sin OpenAI el motor cae a coincidencia por palabras clave.
  if (!provider_configured("openai", config)) {
    add("blocker", "Sin IA real (OpenAI)", "Falta la API key de OpenAI: el bot entiende por palabras clave, no lenguaje libre. Configúrala en Integraciones.");
  }
