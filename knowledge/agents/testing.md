...
- `npm run convo:explain -- <id-o-url>` ([eval/explain_conversation.js](../../eval/explain_conversation.js)): imprime el **diagnóstico turno por turno** de una conversación real (intents+confianza, acciones+status, **ruta de clasificacion** con provider/model + AI/fallback/recoleccion + razones, respuesta del bot, sesión de recolección, llamadas de IA fallidas, guardrails) y marca señales de falla (needs_review, sin respuesta, errores). Reusa `get_conversation_view` (misma verdad que el inspector).
