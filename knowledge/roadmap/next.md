...
2. Implementar mas handlers reales para las actions `stub_*`.
   ...
   - **Generación de documentos/hojas (Google — decisión founder):** `generar_documento` → **Google Doc**, `generar_excel` → **Google Sheet** (compartibles por link en WhatsApp; el usuario exporta a Word/Excel desde ahí). **v1 = service account de YoAyudo** (dueño del archivo en un **Shared Drive** de Workspace, le da acceso al usuario por link/correo); **OAuth por negocio = v2** (doc en el Drive del dueño). Pendiente para v1: integrar Google Docs/Sheets API, **service account JSON + Shared Drive** (prerequisito del founder), mover `generar_excel` a `pending_provider`, registrar `integration_events` con latencia. Doc necesita además IA para el contenido (reusa el AI provider configurable); Excel es datos→hoja, sin IA. Ver `architecture/actions.md`.
...
- **Auth por negocio**: IMPLEMENTADO detras de `AUTH_ENABLED` (default off). Owner de plataforma ve todo; usuarios de negocio loguean en `/login` y solo ven `/dashboard/business/:su_negocio`. Falta para produccion: reset de password, invitaciones, rate-limit de login y roles mas finos (hoy owner/member). Detalle en `IMPLEMENTATION_STATUS.md` seccion Auth.
  - **Bloqueante antes de prender auth (nav del dueño):** la política (`create_auth_policy`, `auth_middleware.js`) hoy confina al usuario de negocio a `/dashboard/business/:org` y redirige `/dashboard/accounts/*` fuera. Con el nuevo nav del dueño (Dashboard/Bots/Canales + review a nivel cuenta) eso deja **inalcanzables** el dashboard de cuenta, las páginas de Bots/Canales y la review owner-facing con auth ON. Hay que extender la política para permitir al usuario de negocio entrar a `/dashboard/accounts/:id(/…)` de cuentas de SU organización (check cuenta→org; la política hoy es síncrona y sin DB, habría que volverla async o cachear el mapeo cuenta→org). Auth OFF por default → hoy no rompe.
