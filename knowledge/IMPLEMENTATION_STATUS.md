...
- Clasificacion de intenciones por AI en el inbound, **on por default para todos los bots** (no opt-in; el selector de Modelo IA ya implica AI): el **provider de AI se resuelve por scope** (bot > cuenta > global > env, ver bullet siguiente) y su `classify_intents` clasifica multi-intent de lenguaje libre. `message_processor` siempre pide AI; el provider decide capacidad (con key → AI real; mock o sin key → keywords). En error de AI degrada a deterministico y registra el fallo en `ai_calls`. `use_ai_classification` es control interno AI/fallback (el reintento lo pone en false), no config de bot. Extraccion de campos sigue determinista.
- **AI provider configurable por scope (global → cuenta → bot)** + **OpenAI / Gemini / Claude**. Resolución `resolve_ai_config({bot, account, global, env})` (`src/ai/ai_config_resolver.js`, helpers en `ai_config_scope.js`): precedencia **bot > cuenta > global > env**, default global, "Heredar" = ausencia/inherit; provider y model salen del MISMO scope. Storage: bot en `definition_json.ai`, cuenta en `accounts.settings_json.ai`, global en tabla `platform_settings` (key `ai_provider`; migración `0020`). El engine YA la usa: `message_processor` resuelve por mensaje y construye el provider (el provider **inyectado en tests gana**, así no se rompen los pipeline tests). Adapters `gemini_provider`/`claude_provider` (extienden `mock_provider`, mismo contrato JSON que openai vía `ai_prompts.js`); el factory devuelve el adapter **solo con key configurada, si no `mock_provider`** (nunca finge ni lanza en construcción). UI en 3 niveles: editor de bot (select con "Heredar" + hint del valor efectivo), inspector de cuenta (`POST /inspector/accounts/:id/ai`), admin (`POST /admin/platform/ai`). Integraciones `gemini`/`claude` en el registry (group `ai`). Sin keys reales, Gemini/Claude caen a mock y el readiness lo marca; las llamadas reales se verifican al configurar keys. **`platform_settings` (global) es compartido**: un default global afecta a los bots que heredan.
- **Integraciones de generación (contratos honestos)**: `generar_imagen`, `generar_documento`, `generar_excel` en el `action_registry` con provider requirements + interacciones en el editor. Mientras no haya proveedor/handler real, el executor devuelve `pending_provider`/`not_implemented` → **guardrail** (nunca finge). `generar_imagen`/`generar_documento` exigen un proveedor de IA (readiness); `generar_excel` es interno (datos→xlsx). Wiring real + subida a S3 pendiente (necesita keys/libs).
- **Inbound multicanal: WhatsApp + Instagram DM + Facebook Messenger** (paridad completa). El engine es **agnóstico de canal**: `process_inbound_message(channel, dependencies, event)` recibe un **descriptor de canal** (`src/channels/channel_registry.js`: `whatsapp_channel`/`instagram_channel`/`messenger_channel`) que abstrae resolución de identidad, envío y descarga de media; el `event` es el mensaje ya normalizado. WhatsApp Cloud API usa `entry[].changes[].value.messages[]`; **Instagram y Messenger comparten** el webhook "Messenger Platform" (`entry[].messaging[]` con `sender`/`recipient`/`message`) → un solo parser (`meta_messaging_parser.js`, ignora echoes/reads/vacíos) y un solo cliente Send API (`meta_messaging_client.js`, `POST /me/messages` con el token de página/cuenta). Identidad: `meta_identity.js` resuelve `recipient.id` (cuenta IG / página FB) → bot/cuenta/org vía `instagram_account_bot_assignments` / `facebook_page_bot_assignments` (espejo de WhatsApp). Webhooks: `GET/POST /webhooks/instagram` y `/webhooks/messenger` (verificación `hub.challenge` + firma `X-Hub-Signature-256` con `META_APP_SECRET`, ack inmediato + proceso async; `meta_webhook_routes.js`). El contacto se identifica por `(account_id, channel, external_id)` — WhatsApp conserva su dedupe por teléfono intacto, IG/Messenger deduplican en JS (PSID/IGSID). **Honesto**: sin token de página el envío queda `not_configured` (registra integration_event, nunca finge); la respuesta del bot se genera y guarda igual. Adjuntos entrantes guardados en los 3 canales (IG/Messenger por la URL del webhook). UI: pestaña Canales del editor con tab Messenger + filas por canal; pastilla de canal en el visor de conversación. Ver `architecture/bot_engine.md` y `database.md`.
- **Recolectar información (entrevista abierta multi-turno, async, con memoria viva)**. Interacción stateful: el bot hace **una pregunta a la vez, derivada de la respuesta anterior** (no es un formulario fijo), hasta que tiene suficiente. Estado vivo en `information_collection_sessions` (migración `0023`): `findings_json` (lo recolectado), `transcript_json`, `last_question`, `status` (`collecting`→`ready`→`completed`/`abandoned`), `turn_count`/`max_turns`, `completion_reason`. **Captura**: mientras hay sesión `collecting`, el engine trata cada mensaje como respuesta (bypassa la clasificación normal — un "vendimos 3200" en medio NO es una venta); al cerrar (`ready`) la conversación vuelve a la normalidad. **Async**: cada webhook = un turno; el estado persiste entre mensajes. Arranca **por frase configurable** (intent `collect_information_start`, p. ej. "arma una propuesta"); gateado a que el bot tenga la interacción habilitada. El turno lo decide la IA (`provider.advance_information_collection`, mock determinístico + OpenAI/Gemini/Claude; sin key cae al banco de preguntas determinístico), orquestado en `src/collection/collection_service.js` (no es un action handler: necesita el provider). Cada turno se loguea en `ai_calls`. **Desacoplado de la generación**: al cerrar guarda el resultado reutilizable; **Modo A** (opción `generar_documento_al_terminar`) dispara `generar_documento` con los `findings`; **Modo B** (default) lo deja en cola y un "genera el documento de eso" (intent `generate_document_request`) lo **consume** después. Honesto: `generar_documento` es stub → `pending_provider`, nunca finge. UI: interacción en el editor (guía en prosa + checkbox de auto-generar) y panel **"Recolección en vivo"** read-only en el visor (estado, objetivo, turnos, findings, pregunta pendiente). Ver `architecture/bot_engine.md` y `database.md`.
- **Adjuntos de conversación (WhatsApp) → S3 / local**. Cuando llega un mensaje inbound con media (imagen/documento/audio/video), el engine descarga el binario y lo guarda: **S3 si hay `CONVERSATION_MEDIA_S3_BUCKET`, si no fallback local** (`.storage/conversation-media`), registrando una fila en `message_attachments` (migración `0021`). Piezas: `whatsapp_client.download_media` (pide la URL temporal de Meta y baja el binario; **gated en credenciales, nunca lanza** → sin token devuelve `{downloaded:false}` y el inbound sigue), `conversation_media_store` (`store_/read_conversation_media`, channel-agnostic, `s3_client`/`config` inyectables), `message_attachment_repository`, y `store_inbound_attachment` en `message_processor` (**best-effort**: cualquier fallo se traga con try/catch + guardrail, jamás rompe el pipeline). La vista de conversación muestra **miniatura** para imágenes y **pastilla con nombre** para otros archivos; el binario se sirve por `GET /inspector/media/:attachment_id` (lee de S3/local, nunca expone la URL de Meta ni la ruta de almacenamiento). Funciona 100% sin keys S3 (local). Aplica a los **tres canales con inbound** (WhatsApp, Instagram, Messenger): cada uno descarga su media (WhatsApp por media id; IG/Messenger por la URL del webhook). Ver `architecture/bot_engine.md` y `database.md`.
...
- **Decision trace por mensaje**: el inbound registra `processing_events.routing_decision` con provider/model resueltos, modo de clasificacion (`ai_requested` / `deterministic_fallback` / `active_collection`), fallback/error si aplica, intents detectados vs efectivos, segmentos y operaciones parseadas con `action_id`. El tab **Ruteo** del trace muestra esta caja negra antes del ruteo de agente/deterministico, y `npm run convo:explain -- <id-o-url>` imprime la ruta por turno.
- **UX de depuración de conversaciones**: la lista `/inspector/bots/:id/conversations` usa la bandeja compacta compartida (`+conversation_list`) en vez de tabla alta: previews a 2 líneas, tarjetas clickeables, rollup de total/revisión/multi-interacción, chips humanizados y badge ámbar de revisión pendiente. En el visor de conversación, el popover del chip de acción muestra la ruta de clasificación cuando existe `routing_decision` (AI/fallback/recolección, modelo, razón y segmentos), sin obligar a abrir el trace completo.
...
- **Readiness del bot** (`compute_bot_readiness`, `src/bots/bot_readiness.js`): un **banner arriba del editor** lista lo que impide que el bot opere DE VERDAD (no basta `status='active'`): sin canal, sin el provider de IA **resuelto** configurado (IA real — el blocker nombra el provider: OpenAI/Gemini/Claude, o "Sin IA real" si resuelve a mock), bot en borrador, o una acción habilitada sin su proveedor externo (p. ej. `buscar_negocios` → Google Places/Yelp/SerpAPI; `responder_con_voz` → ElevenLabs) y stubs `stub_*` aún no implementados. Se deriva de la config real del bot (estado + canales + `acciones_habilitadas_json`) cruzada con `integration_definitions.is_configured` (sin llamadas de red). Bloqueantes (clay) vs avisos (ámbar); lo provee `get_bot_view` como `readiness`.
- `action_audit_logs`.
...
- **Auth opcional por negocio** (`src/auth/`, flag `AUTH_ENABLED`, default OFF = cero cambio de comportamiento): login con email+password en `/login` (scrypt nativo de `node:crypto`, cookie HMAC firmada `yoayudo_session`, sin tabla de sesiones — revocar = cambiar `SESSION_SECRET` o desactivar el usuario). Politica (`auth_middleware.js`): el **platform owner** (`is_platform_owner`) ve todo; un **usuario de negocio** solo `/dashboard/business/:su_organization_id` y subrutas — cualquier otra ruta lo regresa a su negocio. El topbar le muestra **Dashboard + Bots/Canales** (Bots/Canales solo con cuenta en contexto) y le esconde Review/Admin; Inspector ya no va en el header para nadie. **Follow-up (antes de prender auth):** la política hoy redirige `/dashboard/accounts/*` fuera, así que con auth ON las páginas nuevas del dueño (dashboard de cuenta, Bots, Canales, Review) **no serían alcanzables**; hay que permitir al usuario de negocio entrar a `/dashboard/accounts/:id(/…)` de cuentas de su organización (check cuenta→org). Auth OFF por default → hoy no rompe. Ver `roadmap/next.md`. Publicos: `/login`, `/logout`, `/health`, `/public/`, `/webhooks/` (Meta) e `/internal/` (token propio). En production con auth activo, `SESSION_SECRET` es obligatorio (throw al firmar). Seed crea `owner@yoayudo.local`/`yoayudo-owner` (owner global) y `demo@yoayudo.local`/`yoayudo-demo` (usuario del negocio demo). Esto es control de acceso founder-stage, no auth productivo completo (sin reset de password, sin invitaciones, sin rate-limit de login).
...
- Navegacion scopeada por cuenta: **la cuenta es el único scope** (el negocio se deriva de ella, no viaja en la URL). Con una cuenta en contexto el **nav del dueño** se queda en ella: Dashboard → `/dashboard/accounts/:id`, **Bots → `…/bots`, Canales → `…/channels`** (estos dos solo `if nav_ctx`, visibles a todos los roles). **Review (`/review?account=:id`) y Admin (`/admin/integrations`) son del operador** (`unless is_business_user`). **Inspector ya no va en el header** (la página de Bots es la entrada del dueño a cada bot). Lo deriva `navigation_context` (`src/app/navigation_middleware.js`): `nav_context = { account_id }` + `active_nav` (resalta `bots`/`canales` por path `…/bots|channels`, el resto de `/dashboard/*` → `dashboard`). Review (vista global del operador) muestra `.scope-banner`; el inspector no. Ver `architecture/frontend.md`.
...
- Review queue.
- **Eval de conversaciones (golden conversations, `npm run eval`)**: harness aparte de Vitest para medir/optimizar el comportamiento de los bots. Lee fixtures JSON (`eval/conversations/*.json`: `setup` + `turns` con `expect` + `expect_final.db`), corre el **pipeline real** (`handle_*_webhook_payload`) sobre `pg-mem` fresco por fixture contra el **proveedor de IA real** (`--provider=`/`--model=`; sin key avisa y usa mock), y evalúa asserts (intents, reply_contains/matches, actions+status de `action_audit_logs`, no_action/needs_review, db por **delta**). **No es gate de CI**: produce un **dashboard** (`eval/results/report.html` + `latest.json`) con el **% que pasa**. Fixtures `baseline_failing` = corpus que hoy falla ("el bot no supo qué hacer"); el ciclo es subir ese % y promover las que se vuelven verdes. Runner reusable en `eval/eval_runner.js`. **Debug de conversaciones reales**: `npm run convo:explain -- <id-o-url>` imprime el diagnóstico turno por turno de una conversación del inspector, y `npm run convo:export -- <id-o-url>` la convierte en un fixture (con el `expect` precargado con lo que el bot hizo) para fijarla como caso. Ver `agents/testing.md`.
- Tests unitarios e integracion del pipeline, router, memory, inspector, auth, clasificacion por AI, guardrails admin, auto-aprendizaje y Bot Engine comercial. **234 tests / 44 archivos, todos verdes** (Vitest sobre pg-mem). Incluye la recolección de información: `tests/unit/collection_provider.test.js` (turno determinístico del mock: START→primera pregunta, ADVANCE→acumula+siguiente, cierre por usuario/tope/agotar preguntas) y `tests/integration/collection_pipeline.test.js` (arranque por frase, avance acumulando findings, **captura** —un "vendimos 3200" en medio no genera venta—, cierre `ready`+resumen, Modo B consume `pending_provider`, Modo A auto-genera al cerrar, idempotencia). Incluye el inbound de Instagram/Messenger: `tests/unit/meta_messaging_parser.test.js` (normaliza DM IG/Messenger, ignora echoes/reads/vacíos, extrae adjunto por URL), `tests/integration/meta_messaging_pipeline.test.js` (resolvers IG/FB, inbound IG y Messenger → mensaje/conversación/contacto por canal + respuesta + adjunto + dedupe por `(cuenta, canal, external_id)`) y `tests/integration/meta_webhook_routes.test.js` (verificación `hub.challenge` IG/Messenger). Incluye los adjuntos de conversación: `tests/unit/conversation_media_store.test.js` (round-trip local + S3 con cliente inyectado), `tests/unit/whatsapp_media_download.test.js` (sin token → `downloaded:false`, happy path, media sin url), `tests/integration/conversation_media_inbound.test.js` (inbound con media → fila `message_attachments` local; `downloaded:false` → 0 filas sin romper el pipeline) y `tests/integration/conversation_media_view.test.js` (la conversación muestra la miniatura + `GET /inspector/media/:id` sirve el binario; 404 si no existe). Incluye `tests/unit/ai_config_resolver.test.js` (precedencia bot>cuenta>global>env, inherit, provider+model del mismo scope), `tests/unit/ai_providers.test.js` (factory gemini/claude con/sin key → adapter/mock; adapters degradan a mock sin flag, normalizan con fetch mockeado, lanzan sin key), `tests/integration/ai_config_settings.test.js` (platform_settings + merge de `accounts.settings_json.ai` + resolución end-to-end), y casos nuevos de `bot_readiness` (provider resuelto gemini/claude/mock). Incluye `tests/unit/bot_readiness.test.js` (qué le falta a un bot para operar: canal, IA real, proveedores de sus acciones; acciones internas no exigen proveedor). Incluye `tests/unit/lexorank.test.js` (rank_between: orden estricto, inserción repetida en el mismo hueco, 500 inserciones aleatorias sin colisión) y `tests/integration/crm_clients.test.js` (identidad/clave de negocio, dedupe cross-canal, ciclo lead→cliente, captura sin nombre→completar nombre, ejecución por test-message e inbound, guardrail, value_summary, chip "Ver prospecto", render del detalle CRM y **reordenar/mover por rank**). Incluye `tests/integration/review_queue.test.js` (scope + auto-learn de la cola global, vía `review_service` compartido) y, en `operational_dashboard.test.js`, el nav scopeado (Bots/Canales como links de cuenta, Review/Admin operador, sin Inspector), las **páginas dedicadas de Bots/Canales** (render + popups de alta), la **review a nivel cuenta** (métrica "Sin resolver" en el dashboard + página owner-facing + resolver/aprender), el redirect legacy `/dashboard/business/:b/accounts/:a[/...]` → `/dashboard/accounts/:a`, el render unificado del overview (métricas + detalle del día solo en "Hoy") y la acumulación de ventas/compras sobre `?range=7d`.
...
- `0019_crm_pipeline_rank.sql`: columna `pipeline_rank` (text, nullable) + índice `(account_id, pipeline_status, pipeline_rank)` para el **orden manual del tablero CRM** (LexoRank; reorder con drag & drop). Backfill lazy en JS (sin window functions, pg-mem-safe). Ver `architecture/crm.md`.
- `0020_ai_config_settings.sql`: **AI provider por scope**. `ALTER TABLE accounts ADD COLUMN settings_json jsonb DEFAULT '{}'` (cuenta guarda `settings_json.ai`) + tabla genérica `platform_settings (key, value_json, updated_at)` para el default global (key `ai_provider`). DDL dentro del vocabulario de `0013` (compatible pg-mem). No destructiva.
- `0021_message_attachments.sql`: **adjuntos de conversación**. `CREATE TABLE message_attachments` (FK a `messages` ON DELETE CASCADE; columnas `channel`, `provider` s3/local, `bucket`/`s3_key`/`region` o `local_path`, `mime_type`, `size_bytes`, `original_filename`, `source_media_id`, `status`) + índice por `message_id`. No destructiva.
- `0022_messaging_channels.sql`: **Messenger + identidad de contacto por canal**. `CREATE TABLE facebook_pages` + `facebook_page_bot_assignments` (espejo de Instagram), `access_token` en `instagram_accounts`/`facebook_pages` (Send API, nullable), y en `contacts`: `ADD COLUMN channel`/`external_id` + `whatsapp_phone DROP NOT NULL` + índice `(account_id, channel, external_id)`. Sin backfill ni cambios al índice de WhatsApp (path WhatsApp intacto). No destructiva.
- `0023_information_collection_sessions.sql`: **entrevista de recolección con memoria viva**. `CREATE TABLE information_collection_sessions` (FK a `conversations` ON DELETE CASCADE; `objective`, `guidance`, `findings_json`, `transcript_json`, `last_question`, `status`, `turn_count`/`max_turns`, `completion_reason`, `follow_up_action`) + índices por `(conversation_id, status)` y `(account_id, status)`. No destructiva.
...
- `GET /webhooks/instagram` / `POST /webhooks/instagram` (Instagram DM; verificación `hub.challenge` + firma Meta, ack + proceso async)
- `GET /webhooks/messenger` / `POST /webhooks/messenger` (Facebook Messenger; mismo patrón)
- `POST /dev/seed`
...
- `GET /dashboard/accounts/:account_id/bots` · `…/channels` (páginas dedicadas del header, antes pestañas del dashboard; `dashboard/bots.pug` · `dashboard/channels.pug`, con sus popups de alta)
- `GET /dashboard/accounts/:account_id/review` + `POST …/review/:review_item_id/resolve` (review **owner-facing** de la cuenta: mensajes sin resolver en español llano; el dueño responde y —`learn` opt-in— el bot aprende. Reusa `src/review/review_service.js`, compartido con `/review`; métrica "Sin resolver" en el dashboard enlaza aquí)
- `GET /review` (cola **global del operador**; `?account=` filtra los pendientes a una cuenta; muestra `.scope-banner` + link de escape)
...
- `GET /inspector` (NO renderea: el inspector siempre es por cuenta. `?account=` legacy redirige a `/inspector/accounts/:id`; sin cuenta redirige a `/admin/bots`, el overview cross-account de bots — antes caía a `/dashboard` y parecía un link roto)
...
- `GET /inspector/media/:attachment_id` (sirve el binario de un adjunto de conversación desde S3/local; lo usan la miniatura `<img>` y el enlace de descarga del visor; 404 si no existe)
...
- `POST /admin/bots` (crear **bot de sistema** en draft: nombre + descripción → `custom_bot_service.create_system_bot`, en la cuenta oficial donde viven los system; redirige al editor del inspector) · `POST /admin/bots/:bot_id/status` (activar/pausar-a-draft/archivar) · `POST /admin/bots/:bot_id/clone` (copia custom en draft en su cuenta, redirige al editor)
...
- `GET /dashboard/accounts/:account_id/crm` (página CRM: prospectos/clientes en tablero por etapa, **drag & drop**) + `GET /dashboard/accounts/:account_id/crm/:client_id` (detalle scopeado al dashboard, con botón "Ver conversación" + select de etapa) + `POST /dashboard/accounts/:account_id/crm/:client_id/stage` (mover de etapa: select accesible del detalle, deja al final) + `POST /dashboard/accounts/:account_id/crm/:client_id/move` (drag & drop: posiciona entre `before_id`/`after_id` con rank LexoRank, dentro o entre columnas). Entrada desde la métrica "Prospectos". Ver `architecture/crm.md`.
...
- `POST /dashboard/accounts/:account_id/bots` (alta de bot de cuenta: custom con `name`, o clon de bot de sistema con `source_bot_id`; **redirige al editor del bot recién creado** `/inspector/bots/:id`. El custom nace de `minimal_draft_definition` con objetivo/instrucciones **vacíos** — el editor los muestra como placeholder, no relleno; el zod permite `goal`/`operating_instructions` vacíos en draft)
...
- **Llamadas reales a Gemini/Claude**: los adapters existen y el contrato funciona, pero sin API key caen a `mock_provider`. La verificación end-to-end con keys reales (HTTP) queda pendiente (no se testea sin red).
- **Generación real de imagen/documento/excel**: hoy son contratos honestos (registran guardrail `pending_provider`/`not_implemented` hasta que haya proveedor/keys + librerías + subida a S3).
...
- `npm test`: OK, 44 archivos, 234 tests (Vitest).
- `npm run db:migrate`: aplica las migraciones `0001`–`0023` en orden.
