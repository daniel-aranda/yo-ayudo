...
- `GET /inspector` (home "Inspector por bots"; scopea a cuenta vía path `/inspector/accounts/:id`, el `?account=` legacy redirige)
...
Vistas en `src/web/views/admin/` que comparten patrón: `.admin-subnav` (links Integraciones / Interacciones / Bots / Negocios / Conversaciones / Guardrails, `.is-active` el actual), `.period-filter` (24h/7d/30d via `?since_hours=`), tira de `.metric` (con `.metric-hint` de microcopy y tints `.metric--success`/`.metric--warn` para estados) y tabla `.activity-table`. El topbar (`layout.pug`) usa fondo casi opaco (`rgba(250,249,245,.94)` + blur suave) — no translúcido pesado.
