...
- **Generación** (`generar_imagen`, `generar_documento`, `generar_excel`): contratos honestos con interacción en el editor + provider requirements. Sin handler/proveedor real, el `stub_result` del executor devuelve **`pending_provider`** para `generar_imagen`/`generar_documento` (exigen un proveedor de IA: OpenAI/Gemini/Claude) y `not_implemented` para `generar_excel` (interno, datos→xlsx) → **guardrail** en ambos casos; nunca fingen. Wiring real + subida a S3 pendiente.
